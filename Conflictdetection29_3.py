...
# # === Conflict Detection (Single Track Only, pre-Connolly) ===
# conflicts = []
...
# #pd.DataFrame(conflicts).to_csv(os.path.join(BASE_DIR, 'printouts', 'conflicts.csv'), index=False)
...
# # Build a graph of connected stations from the segments_df
# from collections import defaultdict, deque
...
#     print(f"\nTimetable rows for Train {train_id}:")
#     print(train_data[['Station', 'Departs_s', 'Minimum Dwell', 'Stop', 'Passing']])
...
# # Print first 3 trains' segment info
# for train_id in list(detected_segments.keys())[:3]:
#     print(f"\nTrain {train_id}:")
#     for seg in detected_segments[train_id]:
#         print(f"  {seg['from']} → {seg['to']} | enter: {seg['enter']}, exit: {seg['exit']}, "
#               f"track: {seg['track']}, dir: {seg['direction']}, tight: {seg['tight_runtime']}")
